...
- `production`: ambiente de produção. Adicione seus recursos em `environments/production/main.tf`.

## Primeiros passos

1. Entre no ambiente: `cd minha-infra/environments/production`
...
Exemplo de backend S3 a adicionar em `versions.tf`:
```hcl
backend "s3" {
  bucket         = "minha-infra-tfstate"
  key            = "production/terraform.tfstate"
  region         = "us-east-1"
  dynamodb_table = "minha-infra-tflock"
  encrypt        = true
}
```
